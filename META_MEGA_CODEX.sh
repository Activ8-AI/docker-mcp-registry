...
ensure_deps() {
  ...
  if "$PYTHON_BIN" -c 'import pkgutil; required=["fastapi","uvicorn","requests"]; missing=[p for p in required if not pkgutil.find_loader(p)]; import sys; sys.exit(0 if not missing else 1)' >/dev/null 2>&1; then
    :
  else
